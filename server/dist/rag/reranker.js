...
export function rerankResults(results) {
    return results.map(item => {
        const source = SOURCES.find(s => s.id === item.chunk.sourceId);
        let authorityMultiplier = 1.0;
        if (source) {
            if (source.authorityLevel === 'Official BIS')
                authorityMultiplier = 1.4;
            else if (source.authorityLevel === 'Government Notification')
                authorityMultiplier = 1.3;
            else if (source.authorityLevel === 'Empanelled Portal')
