...
import { RecommendationRequest, StandardRecommendation } from '../types/index.js';
...
export function getAllStandards(req: Request, res: Response): void {
  ...
  if (search && typeof search === 'string') {
    ...
    filtered = filtered.filter(st => 
      st.isNumber.toLowerCase().includes(s) ||
...
export function getStandardById(req: Request, res: Response): void {
  const { id } = req.params;
...
export function recommendStandard(req: Request, res: Response): void {
  const body = req.body as RecommendationRequest & { userId?: string };
