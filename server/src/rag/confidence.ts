...
export function calculateConfidence(results: RetrievalResult[]): ConfidenceAssessment {
  if (results.length === 0) {
    return {
      level: 'Low',
      score: 15,
  ...
  if (topScore >= 50) score += 30;
  else if (topScore >= 25) score += 15;

  if (officialSourcesCount >= 2) score += 15;
  if (resultCount >= 3) score += 5;
