...
import { IntentAnalysis } from './intent.js';
...
export function performHybridRetrieval(analysis: IntentAnalysis, topK: number = 5): RetrievalResult[] {
  ...
  for (const chunk of DOCUMENT_CHUNKS) {
    ...
    for (const term of queryTerms) {
      if (chunkText.includes(term)) {
        ...
        if (chunk.documentTitle.toLowerCase().includes(term)) lexicalScore += 10.0;
        if (chunk.tags.some(t => t.toLowerCase().includes(term))) lexicalScore += 7.0;
